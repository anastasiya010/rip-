...
// CreateMedication - создание нового лекарства
func (r *Repository) CreateMedication(medication *ds.Medication) error {
	return r.db.Create(medication).Error
}

// UpdateMedication - обновление лекарства
func (r *Repository) UpdateMedication(medication *ds.Medication) error {
	return r.db.Save(medication).Error
}

// GetMedicationsWithFilters - получение лекарств с фильтрацией по категории и производителю
func (r *Repository) GetMedicationsWithFilters(categoryFilter, manufacturerFilter string) ([]ds.Medication, error) {
	query := r.db.Where("is_deleted = ?", false)

	if categoryFilter != "" {
		query = query.Where("LOWER(category) = LOWER(?)", categoryFilter)
	}

	if manufacturerFilter != "" {
		query = query.Where("LOWER(manufacturer) = LOWER(?)", manufacturerFilter)
	}

	var medications []ds.Medication
	err := query.Find(&medications).Error
	if err != nil {
		return nil, err
	}
	return medications, nil
...
// GetFixedCreatorID - возвращает фиксированный ID создателя (для ЛР3)
func (r *Repository) GetFixedCreatorID() uint {
	return 1
}
